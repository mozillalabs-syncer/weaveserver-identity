...
<xsl:template match="testcase">
		...
		<tr class="outer">              
			...
			<xsl:when test="error">
				<td class="innerCellFailed">
					<div style='height:14px;overflow:hidden'><xsl:attribute name='id'><xsl:value-of select="@name"/></xsl:attribute><pre style='margin-top:0px'><a class='ctl'><xsl:attribute name="href">javascript:expand('<xsl:value-of select="@name"/>')</xsl:attribute>+</a> <xsl:value-of select="error"/></pre></div>
					<div style='display:none'><xsl:attribute name='id'><xsl:value-of select="@name"/>_full</xsl:attribute><pre style='margin-top:0px'><a class='ctl'><xsl:attribute name="href">javascript:expand('<xsl:value-of select="@name"/>')</xsl:attribute>+</a> <xsl:value-of select="error"/></pre></div>
					</td>
			</xsl:when>
			<xsl:otherwise>
